...
namespace PPA.Formatting
{
    /// <summary>
    /// 异步操作辅助类
    /// 提供统一的异步操作执行框架，确保 COM 对象在 UI 线程操作
    /// </summary>
    public static class AsyncOperationHelper
    {
        ...
        private static SynchronizationContext GetUIContext()
        {
            ...
            if (context == null)
            {
                // Office 插件环境中，可能需要手动设置 SynchronizationContext
                // 尝试使用 Application 的同步上下文，如果不存在则创建新的
                try
                {
        ...
        /// </summary>
        /// <remarks>
        /// 在 Office 插件环境中，Ribbon 事件处理已经在 UI 线程中执行，
        /// 所以可以直接执行 COM 操作。使用 Task.FromResult 立即返回已完成的任务。
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// 用于 Ribbon 事件处理等场景，自动记录执行时间并处理异常
        /// </remarks>
        /// <param name="asyncOperation">要执行的异步操作</param>
        /// <param name="operationName">操作名称（用于日志，可选）</param>
