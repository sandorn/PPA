...
namespace VBAApi
{
	...
	public static class VbaManager
	{
		...
		public static void EnsureModuleInitialized(NETOP.Application app)
		{
			...
			ExHandler.Run(() =>
			{
				...
				if(!moduleExists)
				{
					...
					codeModule.AddFromString(vbaCode);
					//presentation.Save();
					Debug.WriteLine($"VBA 模块 '{ModuleName}' 已在内存中创建。");
				} else
				{
					Debug.WriteLine($"VBA 模块 '{ModuleName}' 已存在，跳过创建。");
				}
		...
		private static string GetVbaCodeFromFile()
		{
			...
			{
				// 使用 FileLocator 查找文件
				string vbaFilePath = FileLocator.FindFile("Properties\\TableFormatter.vba");

				if(vbaFilePath!=null)
				{
					_vbaCodeCache=File.ReadAllText(vbaFilePath);
					Debug.WriteLine($"成功从文件加载 VBA 代码: {vbaFilePath}");
					return _vbaCodeCache;
				} else
				{
					// 如果文件不存在，抛出异常
			...
			{
				Debug.WriteLine($"读取 VBA 文件时出错: {ex.Message}");
